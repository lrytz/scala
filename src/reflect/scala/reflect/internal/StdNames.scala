...
trait StdNames {
  ...
  trait TermNamesBase { this: TermNames =>

    private object nameMaker {
      implicit def nt(s: String): NameType = nameType(s)
    }
    import nameMaker.nt
    ...
    val REIFY_FREE_VALUE_SUFFIX: NameType  = "$" + "value" // looks like missing interpolator due to `value` in scopre
    ...
    val asInstanceOf_Ob : NameType     = "$" + "asInstanceOf" // looks like missing interpolator due to Any member in scope
    ...
    val conforms: NameType             = "$" + "conforms" // $ prefix to avoid shadowing Predef.conforms
    ...
    val isInstanceOf_Ob : NameType     = "$" + "isInstanceOf" // looks like missing interpolator due to Any member in scope
  ...
  abstract class TermNames extends Keywords with TermNamesApi with TermNamesBase {
