...
abstract class CPSAnnotationChecker extends CPSUtils with Modes {
  ...
  object plugin extends AnalyzerPlugin {
    ...
    override val annotationChecker = Some(checker)

    override def canAdaptAnnotations(typer: Typer, tree: Tree, mode: Int, pt: Type): Boolean = {
...
*/
  ...
  //          println(same + "/" + base + "/" + known)
    ...
    override def adaptAnnotations(typer: Typer, tree: Tree, mode: Int, pt: Type): Tree = {
     ...
     */
    override def pluginsTypedReturn(typer: Typer, tree: Return, pt: Type, default: Type): Type = {
      val expr = tree.expr
    ...
    override def pluginsTyped(typer: Typer, tree: Tree, mode: Int, pt: Type, tpe: Type): Type = {
