...
class ByteCodeRepository[BT <: BTypes](val classPath: ClassPath, val btypes: BT) {
  ...
  /**
   * Contains ClassNodes of classes being compiled in the current run, and the source file in which
   * they are defined.
   */
  val compilingClasses: concurrent.Map[InternalName, (ClassNode, SourceFileOrigin)] = recordPerRunCache(concurrent.TrieMap.empty)

  /**
   * Cache for ClassNodes parsed from classfiles.
   * The [[ClassFileOrigin]] field stores the classpath element that the node is parsed from.
   ...
   */
  val parsedClasses: concurrent.Map[InternalName, Either[ClassNotFound, (ClassNode, ClassFileOrigin, Long)]] = recordPerRunCache(concurrent.TrieMap.empty)
  ...
  private def limitCacheSize(): Unit = {
    if (parsedClasses.size > maxCacheSize) {
      ...
      val minimalLRU = parsedClasses.valuesIterator.collect({
        case Right((_, _, lru)) => lru
      }).toList.sorted(Ordering.Long.reverse).drop(targetSize).headOption.getOrElse(Long.MaxValue)
      parsedClasses retain {
        case (_, Right((_, _, lru))) => lru > minimalLRU
        case _ => false
      }
    }
  }

  def addCompiled(classNode: ClassNode, sourceFilePath: String): Unit =
    compilingClasses(classNode.name) = (classNode, SourceFileOrigin.fromPath(sourceFilePath))

  def addParsed(classNode: ClassNode, classFileOrigin: ClassFileOrigin): Unit =
    parsedClasses(classNode.name) = Right((classNode, classFileOrigin, lruCounter.incrementAndGet()))

  private def parsedClassNode(internalName: InternalName): Either[ClassNotFound, (ClassNode, ClassFileOrigin)] = {
    parsedClasses.get(internalName) match {
      case Some(Left(w)) => Left(w)
      case Some(r @ Right((classNode, origin, _))) =>
        parsedClasses(internalName) = Right((classNode, origin, lruCounter.incrementAndGet()))
        Right((classNode, origin))
      case None =>
        limitCacheSize()
        val parsedClassAndOrigin = parseClass(internalName)
        parsedClasses(internalName) = parsedClassAndOrigin map { case (node, origin) => (node, origin, lruCounter.incrementAndGet()) }
        parsedClassAndOrigin
    }
  }
   ...
   */
  def classNodeAndOrigin(internalName: InternalName): Either[ClassNotFound, (ClassNode, ClassOrigin)] = {
    compilingClasses.get(internalName) match {
      case Some((c, o)) => Right((c, o))
      case _            => parsedClassNode(internalName)
    }
  }
  ...
  def classNode(internalName: InternalName): Either[ClassNotFound, ClassNode] = {
    compilingClasses.get(internalName) match {
      ...
      case None         => parsedClassNode(internalName).map(_._1)
    }
  }
  ...
  private def parseClass(internalName: InternalName): Either[ClassNotFound, (ClassNode, ClassFileOrigin)] = {
    val fullName = internalName.replace('/', '.')
    classPath.findClassFileAndClasspathElement(fullName) map { case (classFile, classPathElement) =>
      ...
      (classNode, classPathElement)
    } match {
      case Some((node, classPathElement)) => Right((node, ClassFileOrigin.fromPath(classPathElement)))
      case None                           => Left(ClassNotFound(internalName, javaDefinedClasses(internalName)))
    }
  }
}

sealed trait ClassOrigin

sealed trait SourceFileOrigin extends ClassOrigin
object SourceFileOrigin {
  def fromPath(path: String) =
    if (path == "<no file>") UnknownSourceFile
    else SourceFile(path)
}
final case class SourceFile(path: String) extends SourceFileOrigin
case object UnknownSourceFile extends SourceFileOrigin

sealed trait ClassFileOrigin extends ClassOrigin
object ClassFileOrigin {
  def fromPath(path: String) =
    if (path == "<no file>") UnknownClassFile
    else if (path.endsWith(".jar")) JarFile(path)
    else ClassDirectory(path)
}
final case class JarFile(path: String) extends ClassFileOrigin
final case class ClassDirectory(path: String) extends ClassFileOrigin
case object UnknownClassFile extends ClassFileOrigin
