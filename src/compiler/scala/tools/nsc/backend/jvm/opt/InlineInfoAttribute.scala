 ...
 */
case class InlineInfoAttribute(inlineInfo: InlineInfo) extends Attribute(BTypes.inlineInfoClassfileAttributeName) {
  /**
   * Not sure what this method is good for, it is not invoked anywhere in the ASM framework. However,
   * the example in the ASM manual also overrides it to `true` for custom attributes, so it might be
   * a good idea.
   **/
  ...
  override def write(cw: ClassWriter, code: Array[Byte], len: Int, maxStack: Int, maxLocals: Int): ByteVector = {
    ...
    for ((nameAndType, info) <- inlineInfo.methodInfos) {
   ...
   *
   * TODO doc. why we use buf. see also doc of overridden.
   */
  override def read(cr: ClassReader, off: Int, len: Int, buf: Array[Char], codeOff: Int, labels: Array[Label]): InlineInfoAttribute = {
    var next = off

    val version = cr.readByte(next)            ; next += 1
    assert(version == 1, s"unexpected version for the ScalaInlineInfo attribute: $version")

    val hasSelfIsFinal = cr.readByte(next)     ; next += 1
    val isFinal = (hasSelfIsFinal & 1) != 0
    val hasSelf = (hasSelfIsFinal & 2) != 0

    val self = if (hasSelf) {
      val selfName = cr.readUTF8(next, buf)    ; next += 2
      Some(selfName)
    } else {
      None
    }

    val numEntries = cr.readShort(next)        ; next += 2
    val infos = (0 until numEntries).map(_ => {
      val name = cr.readUTF8(next, buf)        ; next += 2
      val desc = cr.readUTF8(next, buf)        ; next += 2

      val inlineInfo = cr.readByte(next)       ; next += 1
    ...
    InlineInfoAttribute(InlineInfo(self, isFinal, infos))
  }
}
...
object InlineInfoAttribute {
   ...
   *   [u1]  isFinal (<< 0), hasInlineAnnotation (<< 1), hasNoInlineAnnotation (<< 2)
   */
 ...
 */
object InlineInfoAttributePrototype extends InlineInfoAttribute(InlineInfo(null, false, null))
