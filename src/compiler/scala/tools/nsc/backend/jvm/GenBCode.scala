...
abstract class GenBCode extends BCodeOptClosu {
  ...
  class BCodePhase(prev: Phase) extends StdPhase(prev) {
    ...
    private val q1 = new java.util.concurrent.LinkedBlockingQueue[Item1]
    ...
    private val q3 = new java.util.concurrent.PriorityBlockingQueue[Item3](1000, i3comparator)
    ...
    class Worker2 extends java.lang.Runnable {
      ...
      private def addToQ3(item: Item2) {
        ...
        val Item2(arrivalPos, _, mirror, plain, bean, lateClosures, _, outFolder) = item
    ...
    def populateDClosureMaps(cnode: asm.tree.ClassNode, masterBT: BType, dClosureEndpoints: Iterable[DClosureEndpoint]) {
      ...
      for(dClosureEndpoint <- dClosureEndpoints) {
        ...
        backend.bcode.Util.makePublicMethod(delegateMethodNode)
