...
// --- Lazy Type Assignment --------------------------------------------------
    ...
    def accessorTypeCompleter(tree: ValDef, isSetter: Boolean) = mkTypeCompleter(tree) { sym =>
      logAndValidate(sym) {
        sym setInfo {
          ...
          pluginsTypeSigAccessor(typer, tree, sym, tp)
        }
      }
    }
    ...
    private def templateSig(templ: Template): Type = {
      ...
      pluginsTypeSig(templateNamer.typer, templ, WildcardType, classTp)
    }
    ...
    private def classSig(cdef: ClassDef): Type = {
      ...
      val pluginsTp = pluginsTypeSig(typer, cdef, WildcardType, res)
    ...
    private def moduleSig(mdef: ModuleDef): Type = {
      ...
      val pluginsTp = pluginsTypeSig(typer, mdef, WildcardType, result)
    ...
    private def methodSig(ddef: DefDef): Type = {
      ...
      pluginsTypeSig(typer, ddef, methResTp, res)
    }
    ...
    private def valDefSig(vdef: ValDef) = {
      ...
      pluginsTypeSig(typer, vdef, if (tpt.isEmpty) WildcardType else result, result)
    ...
    private def typeDefSig(tdef: TypeDef) = {
      ...
      pluginsTypeSig(typer, tdef, WildcardType, res)
    }

    private def importSig(iprt: Import) = {
      val Import(expr, selectors) = iprt
      val expr1 = typer.typedQualifier(expr)
      typer checkStable expr1
      if (expr1.symbol != null && expr1.symbol.isRootPackage)
        RootImportError(iprt)

      if (expr1.isErrorTyped)
        ErrorType
      else {
        val newImport = treeCopy.Import(iprt, expr1, selectors).asInstanceOf[Import]
        checkSelectors(newImport)
        transformed(iprt) = newImport
